...
namespace NeuralNetwork {
  public class Program {
    public static void Main(string[] args) {
      ...
      networkOutputLayer.ExportWeights();
  ...
  public class Layer {
    ...
    public float FeedForward(float[] new_inputs, byte _label) {
      ...
      // pass results through leaky relu
    ...
    public void AddLayer(Layer _layer) {
      if(this.NextLayer != null) {
        this.NextLayer.AddLayer(_layer);
      } else {
        this.NextLayer = _layer;
        _layer.PreviousLayer = this;
      }
    }
  ...
  public class OutputLayer : Layer {
    ...
    public int Max {
      get {
        ...
        for(int i = 0; i < OutputProbabilities.Length; i++) {
          if(OutputProbabilities[i] > max) {
            max = OutputProbabilities[i];
            maxIndex = i;
          }
        }
        return maxIndex;
    ...
    private float[] Softmax(float[] x) {
      float sum = 0;
      for(int i = 0; i < x.Length; i++) { sum += (float)Math.Exp(x[i]); }
    ...
    public void ExportWeights() {
      FileStream fs = File.Create("weights.txt");
      Layer currentLayer = this.PreviousLayer;

      while(currentLayer != null) {
        for(int i = 0; i < currentLayer.Weights.Length; i++) {
           for(int j = 0; j < currentLayer.Weights[i].Length; j++) {
             string data = string.Format("{0:N7}",currentLayer.Weights[i][j]) + ",";
